...
function getAlephOmega() {
	if (
		...
		game.alephOmega = game.alephOmega.add(100000);
	}
}
...
function calcCard() {
	game.maxCard = EN(Math.max(game.factorBoosts - 24, 2) ** calcCardExponent(game.collapseTime))
		.floor()
		.max(game.maxCard);
	return game.maxCard;
}
...
function getManifoldEffect() {
	return (
		Math.max(1, game.manifolds + 1 - game.sing.m) ** (game.sfBought.includes(62) ? 2 : 0.5) *
		(game.iups[4 - 1] === 1 ? 3 : 1) *
...
function getFactorBulk() {
	if (game.OP >= V(game.factorBoosts + 3, 1)) {
		let i = 1;
		while (game.OP >= V(game.factorBoosts + 3 + i, 1) && game.factorBoosts + 3 + i <= 27) {
			i++;
		}
...
function calcTotalOPGain() {
	...
	return (
		...
		(calcOrdPoints(game.ord, game.base, game.over) >= 10 ** 265 ? 1 : calcOPonInfMult())
	);
}

function calcOPonInfMult() {
	return (
		(game.upgrades.includes(5) ? 5 : 1) *
...
function getDecrementyRate(x) {
	return (
		...
		(2 ** game.dups[1]) ** (calcOrdPoints(game.ord, game.base, game.over) < 1e30 ? -1 : 1) *
...
function getIncrementyRate(x) {
	...
	return EN(
		((ordRate * x) / 1000) * 2 ** game.iups[1] * (game.iups[7] === 1 ? getDynamicFactorCap() : 1)
	)
		.times(game.aups.includes(5) ? game.assCard[2].mult : 1)
		.times(
			game.leastBoost <= 1.5 ? getDynamicFactorCap() ** getChalIncrementyCurve(game.chal8Comp) : 1
		);
}
...
function calcFactorShiftTime(n) {
	return Math.max(
		1 / game.shiftAuto.toNumber(),
		Math.min(
			(100 / calcOPPS(n - 1)) * (game.leastBoost <= 1.5 ? 1 : 1) +
				OPtoOrd(getFSCost(n - 1), calcBase(n - 1)) /
					(calcTotalMultWOFactor() *
						((game.upgrades.includes(1) ? 2 : 1) * (1 + (game.upgrades.includes(11) ? 3 : 0))) **
							(n - 1)),
			getFSCost(n - 1) / calcOPPS(n - 1)
		)
	);
}
...
function calcOPPS(fs = game.factorShifts) {
	return (
		(game.upgrades.includes(5) ? 5 ** (game.challenge === 1 || game.challenge === 7 ? 4 : 1) : 1) *
...
function calcFactorBoostTime() {
	...
	for (let i = 1; i < 9; i++) {
		...
		if (i !== 8)
			bfact *=
				(([9, 8, 7, 4, 4, 3, 2][i - 1] + 1 + (game.upgrades.includes(11) ? 3 : 0)) *
					(game.upgrades.includes(1) ? 2 : 1)) **
				[0, 0.5, 0.75, 1][game.challengeCompletion[i - 1]];
		if (i === 8) bfact *= getDynamicFactorCap() ** getChalCurve([game.chal8Comp]);
	}
...
function increaseOrdTier2(x) {
	const bupCom =
		x < BHO / 1e270 ? x * 1e270 : (Math.log10(x / (BHO / 1e270)) / Math.log10(3) + 1) * BHO;
	if (game.ord + game.OP > 1e265 && game.challenge !== 1 && game.challenge !== 7) {
		if (game.ord < BHO && bupCom < BHO) {
			game.ord += bupCom;
			if (game.ord > BHO) game.ord = (Math.log10(game.ord / BHO) / Math.log10(3) + 1) * BHO;
		...
		if (game.OP < BHO && bupCom < BHO) {
			game.OP += bupCom;
			if (game.OP > BHO)
				game.OP = (Math.log10(Math.min(game.OP, 1e308) / BHO) / Math.log10(3) + 1) * BHO;
