...
setInterval(function () {
	if (calcOrdPoints(game.ord, game.base, game.over) >= 1e265) infinity();
}, 50);
...
function factorShift() {
	if (
		game.OP >= getFSCost(game.factorShifts) &&
			...
			if (
				!confirm(
					`Are you sure you want to do a Factor Boost?${
						game.upgrades.includes(8) ? " Don't forget to check Incrementy!" : ""
					}`
				)
			)
				return;
...
function refund() {
	const conf =
		game.bConf.ref === 1
			? confirm("Are you sure you want to refund your booster? You'll reset this Factor Boost!")
			: true;
	if (conf) {
			...
			const id = setInterval(() => {
				...
				if (anim === 300) {
					...
					game.cardinals = game.cardinals.add(
						EN(Math.max(game.factorBoosts - 24, 2) ** 0.5).floor()
					);
