...
let game;
const factorShiftCosts = [200, 1000, 10000, 350000, 1e12, 1e21, 1e100, 1.095e272, Infinity];
...
function increment(manmade = 0) {
	if (manmade === 0 || game.manualClicksLeft >= 0.5) {
		if (manmade === 1 && (game.chal8 === 1 || game.challenge === 6 || game.challenge === 7))
...
function loop(unadjusted, off = 0) {
	...
	if (game.incrementy.gt(0)) game.incrementy = game.incrementy.mul(2);
	...
	for (assCount in game.assCard) {
		...
		game.assCard[assCount].power = game.assCard[assCount].power.add(
			game.assCard[assCount].points
				.pow(game.assCard[assCount].points.log10().pow(0.5).max(2))
				.times(0.001 * ms)
		);
		game.assCard[assCount].mult = game.assCard[assCount].power
			...
			.times(
				game.sfBought.includes(71) && assCount == 2
					? 1 + (getSingLevel() + game.manifolds - game.sing.m - game.spentFunctions) * 0.4
					: 1
			)
	...
	if ((game.succAuto < 1e265 || game.limAuto < 1e265) && !(game.ord >= 3 ** 27 && game.base <= 3)) {
		...
		if (game.autoLoop.succ >= 1000 / tempSucc) {
			if (game.autoLoop.lim >= 1000 / tempLim) {
				...
				game.ord += Math.min(
					...
					game.base *
						Math.floor(game.autoLoop.lim / (1000 / (game.limAuto * limAutoMult * totalMult)))
				);
				game.autoLoop.succ %= 1000 / tempSucc;
				game.autoLoop.lim %= 1000 / tempLim;
			} else if (
	...
	if (
		!chal8Tip &&
		game.chal8 === 1 &&
		calcOrdPoints() >= 1e30 * 1e10 ** (game.base == 5 && game.sfBought.includes(61))
	)
		game.ord =
			game.base ** (game.base * 3 + (game.base == 5 && game.sfBought.includes(61) ? game.base : 0));
	changeDynamic(ms);
	if (game.dynamic < 0) game.dynamic = 0;
	if (ms > 0) {
		...
		if (
			...
			const bupCom = Math.min(
				game.bAutoLoop.max / (1000 / buptotalMute),
				game.leastBoost <= 1e10 ? Infinity : game.bAutoLoop.inf / (1000 / buptotalMute)
			);
			game.bAutoLoop.max %= 1000 / buptotalMute;
			game.bAutoLoop.inf %= 1000 / buptotalMute;
	...
	if (game.upgrades.includes(8)) {
		game.incrementy = game.incrementy.add(getIncrementyRate(ms / 2));
		if (getIncrementyRate(1000).gte(game.maxIncrementyRate) && game.challenge % 2 === 0)
			game.maxIncrementyRate = getIncrementyRate(1000);
	}
	game.cAutoLoop.shift +=
		game.leastBoost <= 12 && game.cAutoOn.shift === 1 ? (ms / 1000) * game.shiftAuto.toNumber() : 0;
	if (game.cAutoLoop.shift >= 1) {
		factorShift();
		game.cAutoLoop.shift %= 1;
	}
	game.cAutoLoop.boost +=
		game.leastBoost <= 12 && game.cAutoOn.boost === 1 ? (ms / 1000) * game.boostAuto.toNumber() : 0;
	if (game.cAutoLoop.boost >= 1) {
		if (
			game.OP >= BHO &&
			game.challenge === 0 &&
			!(game.factorBoosts <= slugMile[slugMile.findIndex(mile => mile < game.leastBoost)])
		)
			factorBoost();
	...
	if (
		...
		collapse();
	const themeSave = `<link rel="stylesheet" href="${
		["style/light", "style/dark", "style/space"][game.theme]
	}.css">`;
...
function render() {
	...
	const ordSub =
		game.ord <= 1e200 || getFBps() / getFBmult() >= 10
			...
			: displayOrd(Math.round(game.ord / 1e270 + 1) * 1e270 - 1e270, 3, 0, 0, 0, 0, game.colors);
	get("hardy").innerHTML = `${colorWrap(
		"H",
		ordColor
	)}<sub>${ordSub}</sub><text class="invisible">l</text>${colorWrap(
		`(${game.base})${
			game.ord >= game.base ** 3 || outSize >= 10 ** 264 || getFBps() / getFBmult() >= 10
				? getFBps() / getFBmult() >= 10
					? `=${game.base}`
					: ""
				: `=${beautifyEN(outSize)}`
	...
	game.canInf =
		...
		outSize >= (game.leastBoost <= 15 ? (game.leastBoost <= 1.5 ? 10 : 100) : 10240) ||
		outSize >= Infinity;
	if (game.infUnlock === 1) {
	...
	if (game.boostUnlock === 1) {
		...
		if (game.challenge === 0 && game.chal8 === 0) {
			...
			get("finishChallenge").innerHTML = `Complete the challenge!<br>${beautify(
				game.chal8 === 1
	...
	if (getFBps() < 10 && game.canInf) {
		...
		if (
			get("infinityButton").innerHTML !== `Markup to gain ${infinityButtonText} Ordinal Points (I)`
		)
			get("infinityButton").innerHTML = `Markup to gain ${infinityButtonText} Ordinal Points (I)`;
		if (get("infinityButton2").innerHTML !== `+${infinityButtonText}`)
			get("infinityButton2").innerHTML = `+${infinityButtonText}`;
	} else {
		get("infinityButton").innerHTML = `Reach ${
			game.leastBoost <= 15 ? (game.leastBoost <= 1.5 ? 10 : 100) : 10240
		} to Markup`;
		get("infinityButton2").innerHTML = `Reach ${
			game.leastBoost <= 15 ? (game.leastBoost <= 1.5 ? 10 : 100) : 10240
		} to Markup`;
	}
	get("challengeSubTab").style.display = game.upgrades.includes(4) ? "inline-block" : "none";
	get("chalFactorWhole").style.display =
		game.boostUnlock == 1 || game.factorShifts == 7 ? "inline" : "none";
	...
	get("succAutoAmount").innerHTML = `You have ${logbeautify(
		game.succAuto
	)} successor autoclickers, clicking the successor button ${
		game.succAuto > 1e265
			? logbeautify(game.succAuto)
			: beautify(game.succAuto * totalMult * succAutoMult)
	} times per second`;
	get("limAutoAmount").innerHTML = `You have ${logbeautify(
		game.limAuto
	)} maximize autoclickers, clicking the maximize button ${
		game.succAuto > 10 ** 265
			? logbeautify(game.succAuto)
			: beautify(game.limAuto * totalMult * limAutoMult)
	} times per second`;
	get("buysucc").innerHTML = `Buy Successor Autobuyer for ${
		game.challenge === 1 || game.challenge === 7
			? game.succAuto === 1
				? "Infinity"
				: "1.000e6"
			: beautify(Math.min(1e260 + game.succAuto, 100 * 2 ** game.succAuto))
	} OP`;
	...
	get("refundBoosters").textContent =
		"Refund back " + beautify(calcRefund()) + " boosters, but reset all factor shifts (R)";
	...
	get("gainBoosters").textContent =
		(getSingLevel() == 1
			? "Gain " +
			  (game.OP >= V(game.factorBoosts + 3) ? getFactorBoostGain() : game.factorBoosts + 1) +
			  " Boosters"
	...
	get("infinityAuto").innerHTML =
		...
		(game.upgrades.includes(3) && game.autoOn.inf == 1 ? beautify(buptotalMute) : 0) +
	...
	get("autoInfButton").textContent =
		"Markup Autobuyer: " +
		(game.upgrades.includes(3) ? (game.autoOn.inf == 1 ? "ON" : "OFF") : "LOCKED");
	get("bup6 current").textContent =
		getBoostFromBoosters(1) < 1000000
			? getBoostFromBoosters(1).toFixed(2)
			: beautify(getBoostFromBoosters(1));
	...
	get("incrementyText2").textContent =
		"You are getting " + beautify(getIncrementyRate(1000)) + " incrementy per second";
	get("iup1").innerHTML =
		"Base Incrementy multiplier is raised to the 1.05<br>Cost: " +
		beautify(10 ** (5 * (game.iups[0] + 1)));
	get("iup2").innerHTML =
		"Double the production of incrementy<br><br>Cost: " + beautify(10 ** (3 * (game.iups[1] + 1)));
	get("iup3").innerHTML =
	...
	get("manifoldAmount").textContent =
		...
		(game.sing.m > 0.5 ? "-" + game.sing.m : game.sing.m == 0 ? "" : "+" + (0 - game.sing.m));
	get("manifoldBoost").textContent = getManifoldEffect().toFixed(3);
	get("changeOrdNotation").textContent =
		"Current Ordinal Notation: " + ["Madore's", "Buchholz's", "Convenient"][game.buchholz];
	get("changeTheme").textContent =
		"Current Theme: " +
		["Light", "Dark", "Space (https://wallpaperplay.com/page-terms)"][game.theme];
	get("changeInt").textContent = "Millisecond Interval: " + game.msint + "ms";
	get("changeOrdLengthLess").innerHTML =
		"Maximum Ordinal Length below " + displayOrd(10 ** 270 * 4) + ": " + game.maxOrdLength.less;
	get("changeOrdLengthMore").innerHTML =
		"Maximum Ordinal Length above " + displayOrd(10 ** 270 * 4) + ": " + game.maxOrdLength.more;
	...
	for (let i = 0; i < 7; i++) {
		...
		get("challenge" + (i + 1) + "Goal").innerHTML =
			"Goal: " + beautify(challengeGoals[i][Math.min(game.challengeCompletion[i], 2)]) + " OP";
		chalbut(i);
	}
	...
	get("decrementyText").textContent =
		"There is " +
		(game.chal8 === 1 ? beautifypower(game.decrementy) + " decrementy and " : "") +
		game.manualClicksLeft +
		" clicks left";
	get("decrementyText").style.display =
		game.chal8 === 1 || game.challenge === 6 || game.challenge === 7 ? "inline" : "none";
	...
	get("cardPow1").innerHTML =
		...
		" ℵ<sub>0</sub> Power (+" +
		beautify(game.assCard[0].points.pow(game.assCard[0].points.log10().pow(0.5).max(2))) +
		"/s)";
	...
	get("cardPow2").innerHTML =
		...
		" ℵ<sub>1</sub> Power (+" +
		beautify(game.assCard[1].points.pow(game.assCard[1].points.log10().pow(0.5).max(2))) +
		"/s)";
	...
	get("cardPow3").innerHTML =
		...
		" ℵ<sub>2</sub> Power (+" +
		beautify(game.assCard[2].points.pow(game.assCard[2].points.log10().pow(0.5).max(2))) +
		"/s)";
	...
	get("dup2").innerHTML =
		"Halve decrementy growth below " +
		(game.buchholz == 2 ? "ω^(ω3)" : "ω<sup>ω3</sup>") +
		", otherwise double it<br>Cost: " +
	...
	get("darkManifoldAmount").textContent =
		beautify(game.darkManifolds) +
		(game.sing.dm === 0 || game.sfBought.includes(11) ? "" : "-" + beautify(getDMSacrafice()));
	...
	get("collapseCardHelp").innerHTML =
		(game.reachedBHO === 1
			? "Next Cardinal in " +
			  beautify(
					Math.max(
						Math.ceil(
							(calcCard().toNumber() + 1) ** (1 / calcCardExponent(game.collapseTime)) +
								24 -
								game.factorBoosts
						),
						1
		...
		"s in collapse)" +
		"<p>Most Cardinals collapsed at once: " +
		beautify(game.mostCardOnce) +
		(game.aups.includes(8)
			? ", providing a constant " +
			  beautify(game.mostCardOnce.times(0.03)) +
			  " Cardinals per second"
			: "") +
		"</p>";
	get("changeOffline").textContent = "Offline Progress: " + (game.offlineProg == 1 ? "ON" : "OFF");
	get("bup10").innerHTML =
		"The base is always five below " + displayOrd(4e270) + "<br><br>73 Boosters";
	get("aup4").innerHTML =
	...
	get("flashIncrementy").textContent =
		"Flashing Incrementy Reminder: " + (game.flashIncrementy == 1 ? "ON" : "OFF");
	...
	get("chal8IncrementyBoost").innerHTML =
		"<br>To Incrementy: x" +
		(getDynamicFactorCap() ** getChalIncrementyCurve(game.chal8Comp)).toFixed(2);
	get("refundConfirmation").textContent =
		"Refund Confirmation: " + (game.bConf.ref == 1 ? "ON" : "OFF");
	get("refundFB").textContent =
		"Factor Boost if possible on Refund: " + (game.bConf.refFB == 1 ? "ON" : "OFF");
	get("chalConf").textContent = "Challenge Confirmation: " + (game.bConf.chal == 1 ? "ON" : "OFF");
	get("chalFB").textContent =
		"Factor Boost if possible on entering a Challenge: " + (game.bConf.chalFB == 1 ? "ON" : "OFF");
	...
	get("fbps").textContent =
		"You should be getting a total of " + commafy(getFBps()) + " Factor Boost(s) per second";
	...
	get("singEffect").innerHTML =
		...
		(getSingLevel() == 69 ? "<br><b>👀 OMG THAT'S THE NICE NUMBER!!! 👀</b>" : "");
	...
	get("collapseConf").textContent =
		"Collapse Confirmation: " + (game.collapseConf == 1 ? "ON" : "OFF");
	get("singularityFunction").textContent =
		game.mostSing < 19.5 ? "Reach level 20 Singularity" : "Singularity Functions";
	...
	get("functions").textContent = `You have ${
		getSingLevel() + game.manifolds - game.sing.m - game.spentFunctions
	} functions.
They are based on your Singularity level.`;
	...
	get(
		"refundPointAmount"
	).innerHTML = `You have ${game.refundPoints} Refund Points<br>You gain them when you Collapse`;
...
function changeDynamic(ms) {
	...
	if (game.challenge == 6 || game.challenge == 7)
		//No update, that was just the previous minor upgrade time to make more studies
		game.dynamic -=
			(10 ** 297 / 2 / (game.upgrades.includes(14) ? 10 ** 299 : 1) / getManifoldEffect()) * ms;
	let capp =
		10 * getDarkManifoldEffect() * (game.aups.includes(6) ? game.assCard[1].mult.toNumber() : 1);
	if (game.dynamic >= capp) game.dynamic = capp;
}

function getDarkManifolds() {
	if (game.decrementy <= game.darkManifolds * Math.log10(game.sfBought.includes(31) ? 2 : 3))
		return;
	if (game.darkManifoldMax == 1) {
		game.darkManifolds = Math.floor(
			game.decrementy / Math.log10(game.sfBought.includes(31) ? 2 : 3)
		);
	} else {
		...
		for (let factorListCounter = 0; factorListCounter < game.factors.length; factorListCounter++) {
			...
			factorListHTML +=
				...
				`)" class="infinityButton">${
					cost === Infinity
						? `Maxed!`
						: `Increase Factor ${factorListCounter + 1} for ${beautify(cost)} OP`
				}</button></li>`;
		}
...
function displayOrd(ord, base = 3, over = 0, trim = 0, large = 0, multoff = 0, colour = 0) {
	if (ord < base && large == 0) {
		...
		let tempvar4 =
			...
			(tempvar == 1
				...
				: (game.buchholz == 2 ? "^(" : "<sup>") +
				  ...
				  (game.buchholz == 2 ? ")" : "</sup>")) +
			(tempvar3 == 1 ? "" : (game.buchholz == 2 && tempvar > 1.5 ? "×" : "") + tempvar3) +
			(ord - tempvar2 * tempvar3 + over == 0 || trim == game.maxOrdLength.less - 1
		...
		let tempvar2 = displayOrd(
			...
			trim + 1,
			1,
			ordMarks[game.buchholz][tempvar][ordMarks[game.buchholz][tempvar].length - 2] == "x",
			colour
		);
		let output = (colour == 1
			? color(
		...
		let tempvar4 = displayOrd(
			...
			trim + 1,
			large,
			ordMarks[game.buchholz][tempvar + 40][ordMarks[game.buchholz][tempvar + 40].length - 2] == "x"
				? 1
				: 0,
			colour
		);
	...
	if (game.challenge != 2 && game.challenge != 7) {
		if (game.factors.length >= 7 && game.OP >= 1e257) {
			...
			for (let i = 0; i < game.factors.length; i++)
				while (game.OP >= Math.pow(10 ** (i + 1), Math.pow(factorCostExp[i], game.factors[i])))
					buyFactor(i);
		}
	}
}
				  ...
				  ((game.upgrades.includes(3) && game.autoOn.inf == 1) || game.leastBoost <= 25)
						? time(
								game.factorBoosts < 24
									? Math.floor((V(game.factorBoosts + getFactorBulk() + 3) - game.OP) / x / 1e270)
									: postBHOproj(x)
						  )
						: Math.floor((V(game.factorBoosts + getFactorBulk() + 3) - game.OP) / 1 / 1e270) +
						  " click cycles");
...
function getSingularity(x) {
	if (
		x == 0 &&
		game.darkManifolds - getDMSacrafice() >=
			1e6 * (game.sfBought.includes(23) ? 4 : 5) ** game.sing.dm
	) {
		game.sing.dm++;
		...
		x == 2 &&
		game.alephOmega.gte(1e20 * (game.sfBought.includes(21) ? 30 : 100) ** game.sing.nw)
	) {
		game.alephOmega = game.alephOmega.minus(
			1e20 * (game.sfBought.includes(21) ? 30 : 100) ** game.sing.nw
		);
		game.sing.nw++;
	}
}
